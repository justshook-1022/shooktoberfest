...
export default async function MePage() {
  ...
  const { data: player } = await supabase.from("players")
    .select("id,first_name,last_name,email,payment_status,team_id")
  ...
  return (
    <main>
      ...
      <div className="page-shell wide">
        <div className="profile-kicker"><span>Signed in with {provider}</span><span>{auth.user.email}</span><form action="/auth/signout" method="post"><button className="text-link" type="submit">Log out</button></form></div>
        {player ? <>
          <PageIntro eyebrow="Your event" title={roster?.team_name || `${player.first_name} ${player.last_name}`} copy={`${player.first_name} ${player.last_name} · ${player.payment_status === "paid" || player.payment_status === "comped" ? "Spot confirmed" : "Payment pending"}`} />
          ...
          </> : <section className="profile-empty"><p className="eyebrow">Pairing pending</p><h2>Your team and tee time will appear here after the draw.</h2></section>}
